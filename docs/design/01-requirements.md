...
| 오류 상황            | HTTP 상태 코드                  | 에러 메시지                                               |
|:-----------------|:----------------------------|:-----------------------------------------------------|
| 상품 상세 조회 중 예외 발생 | `500 INTERNAL SERVER ERROR` | `상품 상세 조회 중 에러가 발생했습니다. Error: {error.getMessage()}` |
...
| 오류 상황            | HTTP 상태 코드                  | 에러 메시지                                               |
|:-----------------|:----------------------------|:-----------------------------------------------------|
| 상품 미존재           | `404 NOT FOUND`             | `상품이 존재하지 않습니다. productId: {productId}`              |
| 상품 상세 조회 중 예외 발생 | `500 INTERNAL SERVER ERROR` | `상품 상세 조회 중 에러가 발생했습니다. Error: {error.getMessage()}` |
...
- 브랜드가 존재 한다면, 브랜드 정보를 반환한다.
- 브랜드가 존재하지 않는다면, 오류를 반환한다..
...
| 오류 상황          | HTTP 상태 코드                  | 에러 메시지                                               |
|:---------------|:----------------------------|:-----------------------------------------------------|
| 브랜드 미존재        | `404 NOT FOUND`             | `브랜드가 존재하지 않습니다. brandId: {brandId}`                 |
| 브랜드 조회 중 예외 발생 | `500 INTERNAL SERVER ERROR` | `상품 상세 조회 중 에러가 발생했습니다. Error: {error.getMessage()}` |
...
|:---------------|:----------------------------|:---------------------------------------------------|
| 상품 미존재         | `404 NOT FOUND`             | `상품이 존재하지 않습니다. productId: {productId}`            |
| 로그인 하지 않은 사용자  | `401 UNAUTHORIZED`          | `로그인 한 사용자가 아닙니다. X-USER-ID: {X-USER-ID}`          |
| 좋아요 등록 중 예외 발생 | `500 INTERNAL SERVER ERROR` | `좋아요 등록 중 에러가 발생했습니다. Error: {error.getMessage()}` |
...
|:---------------|:----------------------------|:---------------------------------------------------|
| 상품 미존재         | `404 NOT FOUND`             | `상품이 존재하지 않습니다. productId: {productId}`            |
| 좋아요 미존재        | `404 NOT FOUND`             | `좋아요 하지 않은 상품 입니다. productId: {productId}`         |
| 비로그인 사용자       | `401 UNAUTHORIZED`          | `로그인 한 사용자가 아닙니다. X-USER-ID: {X-USER-ID}`          |
| 좋아요 취소 중 예외 발생 | `500 INTERNAL SERVER ERROR` | `좋아요 취소 중 에러가 발생했습니다. Error: {error.getMessage()}` |
...
|:------------------|:----------------------------|:--------------------------------------------------------|
| 비로그인 사용자          | `401 UNAUTHORIZED`          | `로그인 한 사용자가 아닙니다. X-USER-ID: {X-USER-ID}`               |
| 본인이 아닌 사용자        | `403 FORBIDDEN`             | `좋아요 한 상품은 본인이 아니면 확인 할 수 없습니다. X-USER-ID: {X-USER-ID}` |
| 좋아요 조회 요청 중 예외 발생 | `500 INTERNAL SERVER ERROR` | `좋아요 조회 요청 중 에러가 발생했습니다. Error: {error.getMessage()}`   |
...
|:--------------|:----------------------------|:--------------------------------------------------|
| 상품 미존재        | `404 NOT FOUND`             | `상품이 존재하지 않습니다. productId: {productId}`           |
| 상품 재고 부족      | `409 CONFLICT`              | `상품 재고가 부족합니다. productId: {productId}`            |
| 비로그인 사용자      | `401 UNAUTHORIZED`          | `로그인 한 사용자가 아닙니다. X-USER-ID: {X-USER-ID}`         |
| 보유 포인트 부족     | `400 BAD REQUEST`           | `보유한 포인트가 부족합니다. X-USER-ID: {X-USER-ID}`          |
| 주문 요청 중 예외 발생 | `500 INTERNAL SERVER ERROR` | `주문 요청 중 에러가 발생했습니다. Error: {error.getMessage()}` |

---

### 주문 내역 조회

- **API:** `GET /api/v1/users/{userId}/orders`
...
|:-----------------|:----------------------------|:-----------------------------------------------------|
| 비로그인 사용자         | `401 UNAUTHORIZED`          | `로그인 한 사용자가 아닙니다. X-USER-ID: {X-USER-ID}`            |
| 본인이 아닌 사용자       | `403 FORBIDDEN`             | `주문 내역은 본인이 아니면 확인 할 수 없습니다. X-USER-ID: {X-USER-ID}` |
| 주문 내역 조회 중 예외 발생 | `500 INTERNAL SERVER ERROR` | `주문 내역 조회 중 에러가 발생했습니다. Error: {error.getMessage()}` |
...
| 오류 상황            | HTTP 상태 코드                  | 에러 메시지                                                  |
|:-----------------|:----------------------------|:--------------------------------------------------------|
| 주문 미존재           | `404 NOT FOUND`             | `주문이 존재 하지 않습니다. orderId: {orderId}`                    |
| 비로그인 사용자         | `401 UNAUTHORIZED`          | `로그인 한 사용자가 아닙니다. X-USER-ID: {X-USER-ID}`               |
| 본인이 아닌 사용자       | `403 FORBIDDEN`             | `주문 상세 내역은 본인이 아니면 확인 할 수 없습니다. X-USER-ID: {X-USER-ID}` |
| 주문 내역 조회 중 예외 발생 | `500 INTERNAL SERVER ERROR` | `주문 상세 내역 조회 중 에러가 발생했습니다. Error: {error.getMessage()}` |
